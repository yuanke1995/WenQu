...
- [ ] graph — buildin/chatbot/graph.py（**挡在 `create_agent_composite_backend` / `create_agent_filesystem_middleware` / `sync_agent_context_skills`，三者均在未搬的 `backends/` 与 `skills/`**，见「挡在后面的依赖」）
...
- [ ] graph — buildin/subagent/graph.py（**挡在 `create_agent_composite_backend` / `create_agent_filesystem_middleware` / `sync_agent_context_skills`，同 buildin/chatbot/graph.py**）
### callbacks（1）
