...
- 🔲 剩余：5 大块，**12 项待办 + 1 项部分完成**（§一 4 / §二 2 / §三 0 / §五 6，另 `[~]`：lifespan；4+2+0+6=12）。**§一 agents 服务侧已全部落地（批次⑫~⑰ → agent_run_service / agent_config_service / agent_run_manifest_service / agent_request_queue_service / agent_request_service / scheduled_agent_service / subagent_run_service / artifact_service）**，`agent_router` 与 `scheduled_agent_router` 已落地（批次㉑）；`agent_invocation_call_router` / `agent_invocation_eval_router` / `agent_invocation_channel_router` 的 §一 阻塞均已解除。剩余 router 仍阻塞在 §一的 `chat_service` / `context_compression_service`（见「挡在后面的依赖」的主动压缩专项）。**服务已能实跑**（启动组件 5/11，见「本地启动与实跑验收」）。
...
> 依据：逐 router 提取 `from <ref>.…` 模块清单，与本工程已有类比对。**已落地**：`scheduled_agent_router`、`agent_router`（批次㉑）。**已可无阻塞直搬**：`agent_invocation_call_router`、`agent_invocation_eval_router`（阻塞项均在 §一 且已落地）。`agent_invocation_channel_router` 另需 `chat_service`。（`knowledge_eval_router` 的检索面 `aquery`、`mcp_router` 的 mcp/service、`skill_router` 的 skills/{service,remote_install} 三个阻塞均已解除并搬完。）
