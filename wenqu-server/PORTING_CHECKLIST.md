...
- 🔲 剩余：5 大块，**10 项待办 + 1 项部分完成**（§一 4 / §二 2 / §三 0 / §五 4，另 `[~]`：lifespan；4+2+0+4=10）。**§一 agents 服务侧已全部落地（批次⑫~⑰ → agent_run_service / agent_config_service / agent_run_manifest_service / agent_request_queue_service / agent_request_service / scheduled_agent_service / subagent_run_service / artifact_service）**，`agent_router` 与 `scheduled_agent_router` 已落地（批次㉑）；`agent_invocation_call_router` / `agent_invocation_eval_router` 已落地（批次㉒），`agent_invocation_channel_router` 的 §一 阻塞已解除（另需 `chat_service`）。剩余 router 仍阻塞在 §一的 `chat_service` / `context_compression_service`（见「挡在后面的依赖」的主动压缩专项）。**服务已能实跑**（启动组件 5/11，见「本地启动与实跑验收」）。
...
| chat_router | ~~`artifact_service`~~ **已搬完（批次⑰）**；仍余 `chat_service` / `context_compression_service`（**二者的引擎面阻塞已解除**：§三 底座 `eae0a77` 提供了构图 / 调用 / 取状态 / 事件流）。**主动压缩专项（2026-09-19 核查）**：`context_compression_service._compress_agent_checkpoint` 调 `compressor.aforce_summarize(values)`，该方法的骨架全在 langchain 的 `SummarizationMiddleware` 基类（`_apply_event_to_messages` / `_determine_cutoff_index` / `_partition_messages` / `_aoffload_inline_media` / `_get_session_id` / `_build_new_messages_with_path` / `_count_tokens`），**本机无 langchain 源码 → 不可照搬**（框架自带的 `SummarizationHook` 语义不等价：无「历史落盘 + 可恢复」机制、cutoff 算法也不同，故不能替代）；另 `AgentStateRepository` 的 `StateGraphPort.updateState` 在 `AgentsGraphPort` 上尚无对应能力。该 service 的**外层**（线程锁 / idle 校验 / agent 与 capability 校验 / 模型解析 / workdir / runtime 生命周期 / token_usage 合并）不依赖上述缺失面，可独立落地——落地时摘要链路按端口隔离并在未装配时显式抛错。 | §一 + §三 |
...
—— 这是解除 `chat_service` / `context_compression_service` 阻塞的前置。
