...
- [ ] graph — buildin/chatbot/graph.py（**阻塞已解除**：所依赖的 `create_agent_composite_backend` / `create_agent_filesystem_middleware` / `sync_agent_context_skills` 随批次⑩ 全部落地。剩余为构图装配：`create_agent` → `GraphFactory`、`TodoListMiddleware` → 框架 `interceptor/todolist`、`PatchToolCallsMiddleware` → `PatchToolCallsInterceptor`；依赖的 middlewares 10/10 与 toolkits 已就位，属下一批）
...
- [ ] graph — buildin/subagent/graph.py（**阻塞已解除**：同 buildin/chatbot/graph.py，三个 backends 构图函数均已落地；属下一批）
### callbacks（1）
