# 沙盒 provisioner（对应参考实现 docker/sandbox_provisioner/app.py，逐字照搬）。
#
# 品牌词处理（三分法）：
#   - 容器前缀与 Docker/K8s label（yuxi-sandbox / yuxi-sandbox-provisioner）→ wenqu-*：
#     由本进程自产自销（写 label 与按 label 过滤同源），grep 参考仓库无第三方消费方。
#   - Kubernetes 资源名默认值（K8S_NAMESPACE=yuxi-know / SKILLS_PVC=yuxi-skills /
#     USER_DATA_PVC=yuxi-user-data）**保留原值**：它们是外部集群里已存在的资源标识，
#     非自产自销，改名会指不到资源；且均为 os.getenv 默认值，部署侧可覆盖。
#   - FastAPI title 随品牌改名（仅 OpenAPI 文档展示，无消费方）。
...
class LocalContainerProvisionerBackend:
    def __init__(self):
        ...
        self._container_prefix = os.getenv("DOCKER_SANDBOX_PREFIX", "wenqu-sandbox")
    ...
    def _has_expected_network_ownership(network, sandbox_id: str) -> bool:
        ...
        return (
            labels.get("managed-by") == "wenqu-sandbox-provisioner"
    ...
    def _create_network(self, network_name: str, sandbox_id: str):
        ...
        labels = {
            "managed-by": "wenqu-sandbox-provisioner",
    ...
    def list(self) -> list[SandboxRecord]:
        containers = self._client.containers.list(
            all=True,
            filters={
                "label": ["app=wenqu-sandbox", "managed-by=wenqu-sandbox-provisioner"]
            },
        )
...
class KubernetesProvisionerBackend:
    ...
    def list(self) -> list[SandboxRecord]:
        pod_list = self._core_api.list_namespaced_pod(
            ...
            label_selector="app=wenqu-sandbox",
        )
...
app = FastAPI(title="WenQu Sandbox Provisioner", lifespan=lifespan)
