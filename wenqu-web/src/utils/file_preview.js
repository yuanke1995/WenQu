...
export const normalizePreviewResponse = async (response, baseFile = {}) => {
  ...
  const previewType =
                             // 品牌前缀替换：参考实现原名 x-{旧前缀}-preview-type，此处与前端读取端同步改名；
                             // 若将来搬运后端生产者（werk 端文件预览响应头），必须沿用同一字面量，否则预览类型静默回落。
    response?.headers?.get?.('x-wenqu-preview-type') || getPreviewTypeByContentType(contentType)
