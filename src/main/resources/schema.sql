...
CREATE TABLE IF NOT EXISTS `c_ai_knowledge_base` (
    ...
    `chunk_params` TEXT         DEFAULT NULL COMMENT '分块参数(JSON: {"chunk.maxChunks":"300","chunk.overlap":"80",...}; 空=继承全局分块设置)',
...
CREATE TABLE IF NOT EXISTS `c_ai_document` (
    ...
    `processing_params` TEXT   DEFAULT NULL COMMENT '本次解析所用参数快照(JSON；记录分块策略与生效参数，便于回溯"这批块是怎么切出来的")',
