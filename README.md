...
2. **对话**（侧边栏「对话」或「新建对话」）：欢迎页是「渠」字标 +「有什么可以帮你？」+ 推荐问题标签（点击直接提问）。输入框提示「问点什么？输入 @ 指定参考文档；Enter 发送，Shift+Enter 换行」，支持**拖入（显示「松开以添加图片」遮罩）/ 粘贴 / 点击上传**发图（最多 5 张，缩略图可点开预览、单张移除）；工具行从左到右为**智能体下拉**（「默认（全局配置）」或某个预设，底部「管理智能体」跳转配置页）、**上传图片**、**深度思考开关**（灯泡图标，状态记忆），右侧显示本轮实际生效的**模型名**与**发送/停止**按钮。输入 `@` 弹出文档候选（↑↓ 选择 · Enter 确认 · Esc 关闭），被 @ 的文档优先参考，已选引用以 chip 显示并可一键移除。
   - **提问后**：混合检索 + 查询改写 + 知识块关联扩散 → 流式回答（检索阶段先出阶段提示，如「正在思考中…」），位置级插入文档原图、句末 `[N]` 引用角标（点击弹窗看来源全文与图片）、末尾「猜你想问」；开启深度思考时回答上方先出 **「深度思考」折叠面板**流式展示思维链，完成后显示「已完成」徽标（可随时展开回看）；工具调用（开启智能体/工具时）在回答内以状态行实时反馈，工具名按用途友好显示（`知识库精确检索` / `生成文件产物` / `算术计算` / `获取当前时间` / `日期相差天数`）并附耗时或失败标记；产物交付以可下载卡片（文件名 + 描述）下发。
   ...
   - **消息操作**（悬浮在回答下方）：**复制 / 有帮助 / 没帮助（单选锁定，刷新或重进会话仍保持）/ 重新生成 / 更多**（检索调试、导出 Markdown、删除本轮对话），并显示本轮 token 估算与时间；用户消息可**编辑后重新发送**。其中「检索调试」弹窗（标题「检索调试（为什么这么答）」）可按问题分步折叠查看**关键词命中 / 向量命中 / 合并后 / 重排后 / 最终上下文 / 被排除**，「导出 Markdown」导出单轮问答。
...
4. **数据看板**（侧边栏「数据看板」，页头有「刷新」）：核心指标卡（**问答总数 / 无命中率 / 有引用标注 / 反馈满意率**，满意率附 👍👎 明细）+ **检索质量自动体检**（定时按线上参数跑评估集，指标较上期下滑即预警；卡内可点「立即体检」，状态为 正常 / 检索质量下滑 / 待评估集 / 执行异常）+ 热门问题 TOP10（每行「设为推荐」加入欢迎页推荐问题池）+ 无命中问题 TOP10（标注「建议补充知识库」）+ **差评样本（反馈回流）**（还原问题与引用过的知识块，可「加入评估集」或「补知识块」）+ **知识库缺口管理**（无命中问题汇总，点「入库」打开「补充知识块」弹窗——问题自动作标题，填写回答内容与可选的关联文档 ID，确认后写入并自动生成向量）。

5. **系统设置**（侧边栏「系统设置」）：左侧是分组导航（**智能问答模型 / 视觉模型 / 文档解析 / 向量模型 / 检索设置 / 上下文控制 / 深度思考 / 工具调用 / MCP 外部工具 / 语义缓存 / 接口限流 / 定时维护 / API Key 管理 / 技能 Skills / 并行检索**），每组有说明条，标题旁是「恢复本组默认」（向量模型 / 定时维护 / API Key 管理 / 技能 Skills 四组不提供）；**改动后点页头右上角「保存配置」一次性保存全部改动**（按钮上显示待保存项数，未保存时页头提示「有 N 项已修改未保存」，保存成功提示「配置已保存并生效」；参数旁的 `?` 为说明，调试类开关带橙色「调试」徽标）。**问答 / 视觉 / 向量三类模型均可跨厂商热切换**（网关地址 / API Key / 模型名 / 接口路径，API Key 以 RSA 加密入库，页面仅回显 `****后4位`），模型名下方有**厂商预设**下拉（DeepSeek / 智谱 GLM / 阿里百炼 Qwen / Kimi / 豆包 / 腾讯混元 / 百度千帆 / MiniMax / SiliconFlow / 本地 Ollama 等），选中即自动填好网关地址与补全路径；保存即生效免重启。**向量模型**保存前先探测新配置（不可达或维度非法一律拒绝保存），通过后自动重建索引并后台全量重嵌入，面板内展示探测结果、重嵌入进度/维度变化/耗时/索引对账并可手动重试（期间向量检索降级关键词路，服务不中断）；检索权重与行为参数、上下文预算、解析与分块、深度思考增强、限流阈值、语义缓存等均在此在线调整；**工具调用 / MCP 外部工具 / 技能 Skills / 并行检索**四组为能力开关，默认关闭，开启后对话链路才会挂载对应工具（MCP 组支持增删服务、全部重连与临时探测，服务列表也可切「高级编辑（原始 JSON）」维护）；**API Key 管理**为第三方系统签发仅具问答链路权限的 Key（明文仅签发时返回一次，可改名 / 停用 / 删除，页面附「如何使用」curl 示例）。

6. **检索评估**（侧边栏「检索评估」）：页头显示当前评估集用例数（`N 条用例`）与生成时间，按钮**「重新生成评估集」**从真实问答记录重建（问题 → 期望知识块，差评案例自动补充）；页内 **「怎么用（三步）」** 说明卡默认收起、点击展开。「运行配置」里设置 **K 值**（默认 `5,10,20`）与**每次最多**用例数（默认 100），并按需增删**参数组**（每组可选模式「单路 / 多路」，可覆盖 向量权重 / 关键词权重 / 标题奖励 / 向量阈值 / 关键词上限 / topK / 重排下限 / 重排上限 / 重排开关——未拨动的参数框灰色占位回显当前线上值，拨动即为该组显式覆盖；「＋添加参数组」可从预设挑（关键词优先 / 向量优先 / 向量+重排 / 多路模拟 / 空白自定义），最多 8 组）；然后走两条路——**「一键体检（当前配置）」**（用当前配置全量跑，给红绿灯结论与落空题清单，零参数门槛）或**「运行对比评估」**（多组并行对比）。结果表按 **recall@k / MRR / 命中率** 与基线 ↑↓ 对比并给出自动结论（另附弃用文档断言），逐问题明细可展开看「期望块 vs 实际命中 Top10」；表现好的组点**「应用此组」**（二次确认）直接写入线上配置生效，不污染评估。

7. **智能体**（侧边栏「智能体」，管理员；**下列能力默认全部关闭**，需先在系统设置的「工具调用 / MCP 外部工具 / 技能 Skills / 并行检索」分组开启）：列表以卡片展示（名称、`默认` / `子` 标签、描述、模型或「跟随全局」、知识库范围与已开启的能力），顶部支持搜索（`搜索名称或描述`）、刷新与「新建智能体」，卡片上可**配置 / 设为默认 / 删除**。点「配置」或「新建智能体」进入整页表单，顶部实时显示生效摘要（名称 · 模型 · 知识库范围 · 能力），分区填写——**身份**（**用途**：主智能体 / 子智能体；名称、描述）、**模型与提示词**（模型留空跟随全局；系统提示词留空沿用全局，填写后完全替换）、**知识库范围**（全部文档 / 指定文档，指定时勾选允许检索的文档，一篇不选则该智能体检索不到内容）、**能力**（**知识库检索 / 内置高频工具 / 产物交付 / 技能 Skills / MCP 外部工具** 五项，默认「跟随全局」，需要为该智能体单独破例时才改成「开启 / 关闭」，或选「指定」逐项挑；被覆盖的项会标「已覆盖」并显示全局状态）、**子智能体委派**（仅主智能体显示：勾选允许委派的子智能体，最多 4 个；不选则沿用系统的多视角并行检索）、**默认**（仅主智能体显示：设为默认智能体，对话页打开时预选，同一时间只有一个）。主智能体可把复杂问题**并行委派**给子智能体（各自「检索 + 提炼」再由汇总节点合并，去重与摘要在「并行检索」分组配置），工具调用过程在回答内以状态行实时反馈。
